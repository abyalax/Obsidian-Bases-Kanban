...
import { CardDetailPanel } from "./cardDetailPanel.ts";
...
import {
  ...
  formatTags,
  getTagColor,
} from "./utils/propertyFormatters.ts";
...
export function isColumnColors(
  ...
  return (
    ...
    Object.values(value).every(
      (v) =>
        isRecord(v) && Object.values(v).every((c) => typeof c === "string"),
    )
  );
}

export function isCardOrders(
  value: unknown,
): value is Record<string, Record<string, string[]>> {
  return (
    isRecord(value) &&
    ...
    Object.values(value).every(
      (v) =>
        isRecord(v) &&
        !Array.isArray(v) &&
        Object.values(v).every((a) => Array.isArray(a)),
    )
  );
}
...
export class KanbanView extends BasesView {
  ...
  private readonly legacyData: LegacyData | null;
  private groupByPropertyId: BasesPropertyId | null = null;
  private readonly _columnSortables: Map<string, Sortable> = new Map();
  private _entryMap: Map<string, BasesEntry> = new Map();
  private columnSortable: Sortable | null = null;
  private readonly _debouncedRender: DebouncedFn<() => void>;
  ...
  private readonly _controller: QueryController;
  // Card detail panel (Asana/GitHub style slide-in)
  private _detailPanel: CardDetailPanel | null = null;
  ...
  constructor(
    ...
    super(controller);
    this._controller = controller; // store for CardDetailPanel
  ...
  private _persistConfigKey<T>(
    ...
    if (
      JSON.stringify(all[this._prefsPropertyId]) !== JSON.stringify(newValue)
    ) {
  ...
  private render(): void {
    try {
      ...
      if (
        ...
        !availablePropertyIds.includes(this.groupByPropertyId)
      ) {
      ...
      this._entryMap = new Map(
        entries.map((e: BasesEntry) => [e.file.path, e]),
      );

      // Group entries by group by property value
      const groupedEntries = this.groupEntriesByProperty(
        entries,
        this.groupByPropertyId,
      );
      ...
      groupedEntries.forEach((columnEntries, value) => {
        ...
        if (savedOrder) {
          groupedEntries.set(
            value,
            this.applyCardOrder(columnEntries, savedOrder),
          );
        }
      });
      ...
      const newValues = liveValues.filter(
        (v) => !this._prefs.columnOrder.includes(v),
      );
      ...
      const existingBoard = this.containerEl.querySelector<HTMLElement>(
        `.${CSS_CLASSES.BOARD}`,
      );
      if (
        !existingBoard ||
  ...
  private patchColumnCards(
    ...
    const headerEl = columnEl.querySelector<HTMLElement>(
      `.${CSS_CLASSES.COLUMN_HEADER}`,
    );
    if (headerEl) {
      const columnValue = columnEl.getAttribute(DATA_ATTRIBUTES.COLUMN_VALUE);
      const existingRemoveBtn = headerEl.querySelector(
      ...
      body.querySelectorAll(`.${CSS_CLASSES.CARD}`).forEach((card) => {
        const path =
          card instanceof HTMLElement
            ? card.getAttribute(DATA_ATTRIBUTES.ENTRY_PATH)
            : null;
  ...
  private groupEntriesByProperty(
    ...
    entries.forEach((entry) => {
      try {
        ...
        const uncategorizedGroup = ensureGroupExists(
          grouped,
          UNCATEGORIZED_LABEL,
        );
  ...
  private createCard(entry: BasesEntry): HTMLElement {
    ...
    for (const propertyId of order) {
      ...
      // Check if this is a tags property by looking at the label
      const isTagsProperty =
        label.toLowerCase().includes("tag") ||
        propertyId.toLowerCase().includes("tag");
      // Check if this is a date property by looking at the label
      const isDateProperty =
        label.toLowerCase().includes("date") ||
        propertyId.toLowerCase().includes("date") ||
        label.toLowerCase().includes("deadline") ||
        propertyId.toLowerCase().includes("deadline") ||
        label.toLowerCase().includes("due") ||
        propertyId.toLowerCase().includes("due");

      if (isTagsProperty) {
        // Render tags as chips
        console.debug(
          `Tags property detected: ${propertyId} (${label}), value:`,
          value,
        );
        const tags = formatTags(value);
        console.debug(`Formatted tags:`, tags);
        const tagsContainer = valueEl.createDiv({
          cls: CSS_CLASSES.CARD_PROPERTY_TAGS,
        });

        tags.forEach((tag) => {
          const tagEl = tagsContainer.createSpan({
            cls: CSS_CLASSES.CARD_PROPERTY_TAG,
            text: tag,
          });
          // Set custom color for each tag
          const color = getTagColor(tag);
          console.debug(`Tag ${tag} color:`, color);
        ...
        console.debug(
          `Date property detected: ${propertyId} (${label}), value: ${valueStr}`,
        );
        const formattedDate = formatDateIndonesian(valueStr);
        console.debug(`Formatted date result: ${formattedDate}`);
        valueEl.createSpan({
          cls: CSS_CLASSES.CARD_PROPERTY_DATE,
          text: formattedDate,
        });
      } else if (this.app && valueStr.includes("[[")) {
    ...
    cardEl.addEventListener("mouseenter", () =>
      cardEl.classList.add(CSS_CLASSES.CARD_HOVER),
    );
    cardEl.addEventListener("mouseleave", () =>
      cardEl.classList.remove(CSS_CLASSES.CARD_HOVER),
    );
    ...
    const clickHandler = (e: MouseEvent) => {
      ...
      // If already showing this card, close the panel (toggle behavior)
      if (
        this._detailPanel?.getCurrentEntryPath() === filePath &&
        this._detailPanel.isOpen()
      ) {
        this._detailPanel.close();
      ...
      if (this._detailPanel) {
        this._detailPanel.open(entry);
      } else if (this.app?.workspace) {
        // Fallback: open file normally if panel not available
  ...
  private applyColumnColor(
    ...
    if (colorName) {
      const cssVar =
        COLOR_PALETTE.find((c) => c.name === colorName)?.cssVar ?? null;
      if (cssVar) {
  ...
  private removeColumn(value: string, columnEl: HTMLElement): void {
    if (!this._prefsPropertyId) return;
    this._prefs.columnOrder = this._prefs.columnOrder.filter(
      (v) => v !== value,
    );
    this._persistPrefs();
    ...
    this.containerEl.querySelectorAll(selector).forEach((columnBody) => {
      ...
      const value =
        colEl instanceof HTMLElement
          ? colEl.getAttribute(DATA_ATTRIBUTES.COLUMN_VALUE)
          : null;
      if (!value) return;
  ...
  private async handleCardDrop(evt: Sortable.SortableEvent): Promise<void> {
    ...
    const oldColumnValue =
      oldColumnEl instanceof HTMLElement
        ? oldColumnEl.getAttribute(DATA_ATTRIBUTES.COLUMN_VALUE)
        : null;
    const newColumnValue = newColumnEl.getAttribute(
      DATA_ATTRIBUTES.COLUMN_VALUE,
    );
      ...
      Array.from(bodyEl.querySelectorAll(`.${CSS_CLASSES.CARD}`))
        .map((c) =>
          c instanceof HTMLElement
            ? c.getAttribute(DATA_ATTRIBUTES.ENTRY_PATH)
            : null,
        )
    ...
    try {
      const valueToSet =
        newColumnValue === UNCATEGORIZED_LABEL ? "" : newColumnValue;
  ...
  private findCardEl(path: string): HTMLElement | null {
    return (
      Array.from(
        this.containerEl.querySelectorAll<HTMLElement>(`.${CSS_CLASSES.CARD}`),
      ).find((el) => el.getAttribute(DATA_ATTRIBUTES.ENTRY_PATH) === path) ??
      null
    );
  }
  ...
  private setActiveCard(path: string | null): void {
    if (this._activeCardPath) {
      this.findCardEl(this._activeCardPath)?.classList.remove(
        CSS_CLASSES.CARD_ACTIVE,
      );
    }
  ...
  private reapplyActiveCard(): void {
    ...
    this.findCardEl(this._activeCardPath)?.classList.add(
      CSS_CLASSES.CARD_ACTIVE,
    );
  }
