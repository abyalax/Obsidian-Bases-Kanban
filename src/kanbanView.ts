import type { BasesAllOptions, BasesEntry, BasesPropertyId, QueryController } from 'obsidian';
...
import { formatDateIndonesian, formatTags, getTagColor } from './utils/propertyFormatters.ts';
...
export class KanbanView extends BasesView {
	...
	private createCard(entry: BasesEntry): HTMLElement {
		...
		for (const propertyId of order) {
			...
			const isTagsProperty = label.toLowerCase().includes('tag') || propertyId.toLowerCase().includes('tag');
			...
			if (isTagsProperty) {
				...
				console.debug(`Tags property detected: ${propertyId} (${label}), value:`, value);
				...
				console.debug(`Date property detected: ${propertyId} (${label}), value: ${valueStr}`);
		...
		const clickHandler = (e: MouseEvent) => {
			...
			if (this._detailPanel?.getCurrentEntryPath() === filePath && this._detailPanel.isOpen()) {
