...
export class CardDetailPanel {
	...
	private renderContent(entry: BasesEntry): void {
		...
		for (const propertyId of order) {
			const value = entry.getValue(propertyId as `note.${string}` | `formula.${string}` | `file.${string}`);
			...
			const propertyName = propertyId.includes('.') ? propertyId.split('.')[1] : propertyId;
			...
			if (valueStr.includes('[[')) {
				void MarkdownRenderer.render(this.getApp(), valueStr, valueEl, entry.file.path, null);
			} else {
